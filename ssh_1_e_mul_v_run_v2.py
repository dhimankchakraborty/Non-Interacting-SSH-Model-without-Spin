...
for e_v_arr in all_e_val_arr.transpose():
    plt.plot(v_arr, e_v_arr)
plt.grid()
plt.show()


data_file_name = "ssh-1-e-mul-v-run.h5"
data_file_output_directory = "data"
group_name = f"{N}-{v_0}-{v_end}-{v_step_no}-{w}-{topo_state_no}-{datetime.datetime.now().strftime("%d-%m-%y-%H-%M-%S")}"
